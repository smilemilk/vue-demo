 ...
 */
var RSA = function() {
    if(typeof this.RSAUtils === 'undefined')
        var RSAUtils = this.RSAUtils = {};
...
//
    this.BarrettMu = function(m) {
        let _this = this;
        _this.modulus = this.biCopy(m);
        _this.k = this.biHighIndex(this.modulus) + 1;
        var b2k = new BigInt();
        b2k.digits[2 * this.k] = 1; // b2k = b^(2k)
        _this.mu = this.biDivide(b2k, this.modulus);
        _this.bkplus1 = new BigInt();
        _this.bkplus1.digits[this.k + 1] = 1; // bkplus1 = b^(k+1)
...
//
    var RSAKeyPair = function(encryptionExponent, decryptionExponent, modulus) {
        let _this = this;
        _this.e = this.biFromHex(encryptionExponent);
        _this.d = this.biFromHex(decryptionExponent);
        _this.m = this.biFromHex(modulus);
        // We can do two bytes per digit, so
        // chunkSize = 2 * (number of digits in modulus - 1).
        // Since biHighIndex returns the high index, not the number of digits, 1 has
        // already been subtracted.
        _this.chunkSize = 2 * this.biHighIndex(this.m);
        _this.radix = 16;
        _this.barrett = new this.BarrettMu(this.m);
    };
//
    this.getKeyPair = function(encryptionExponent, decryptionExponent, modulus) {
        return new RSAKeyPair(encryptionExponent, decryptionExponent, modulus);
    };
//
//     if(typeof this.twoDigit === 'undefined') {
...
RSA.prototype.getKeyPair = function() {
